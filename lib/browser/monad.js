 ...
 * executes fn (if provided) passing only the current item's id, after traversal
 * returns message string if a cycle is detected
 */
function resolve(id, fn, visited) {
...
//console.log('defining ' + id + ' as ' + filename);
...
console.log('exec: ' + monad.module.id);
...
function load(config) {
  assert(config.monad, 'load() requires config.monad property');
  assert(config.filename, 'load() requires config.filename property');
  assert(config.alias, 'load() requires config.alias property');

  var filename = config.filename;
  var alias = config.alias;
  var globalName = config.globalName;
  var monad = config.monad;
  
  var cache = load.cache || (load.cache = loadcache());
    
  //console.log(filename + ' for ' + monad.module.id);

  var cached = filename in cache;
  var registered = filename in registry;
  ...
  if (!cached && !registered) {
    ...
    s.onload = s.onreadystatechange = function (e) {
      ...
      if (!rs ||  rs == 'complete') {
        ...
        console.warn(s.src + ': ' + monad.module.id);
        var exports = registry[s.src].module.exports;
        monad[alias] = (globalName && global[alias]) || exports;
    ...
    pending[monad.module.id][filename] = filename;
    pending[monad.module.id].push(filename);

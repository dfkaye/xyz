/////////////////////////////////////////////////////////////////
//
// TO DO as of 21 MAY --16 MAY 2014--
//
//  - refactor "monad" to "currier"
//  - add context member to currier, don't make monad/currier the context
...
//  - get rid of state vars (registry, stack, main, scripts, script, etc.)
//  - move stack to exec.stack
//  - move registry to define.cache
...
global.require || (function() {
  function require(request) {
    return require.cache[request] && require.cache[request].exports;
  };
...
/*
 * ready, require, load, attach, & callback to require
 */
...
script.ready = function ready(fn, monad) {
  /////////////////////////////////////////
  //
  // CROSSPLATFORM ISSUE BUBBLING UP....
  //
  /////////////////////////////////////////
  var p = script.pending && script.pending[monad.module.id];
 ...
 */
script.require  = function require(request) {

  console.log('---------------script.require-----------------------');

  assert(request.monad, 'script.require: missing request.monad property');
  assert(request.filename, 'script.require: missing request.filename property');
  assert(request.alias, 'script.require: missing request.alias property');
  ...
  // POSSIBLE FOOTGUN
  // 
  // script.pending is created only if script.require is called
  //
  // ready() down below must refer to script.pending && script.pending[id]
  //
  ////////////////////////////////////////////////////
...
var registry = {}; // global.define.cache
var stack = []; // should be property of exec()
...
function exec(fn, monad) {
   ...
   */
  if (script && !script.ready(fn, monad)) {
    return;
  }

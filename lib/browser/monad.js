...
//
//  - get rid of state vars (main, scripts, script, etc.)
//  - refactor "monad" to "loader"
 ...
 *
 *
 * requires:  assert, window, document, Module, graph, define.cache, assert, script if document
 */
 ...
 *
 * requires:  assert, normalize, PREFIX
 */
 ...
 *  TODO: normalize(request) first
 *
 * requires: main.parentNode, exec, assert
 */
...
script.require  = function require(request) {
  ...
  script.load(request.filename, function callback(err) {
    ...
    if (!err) {
      ...
      exports = define.cache[filename].context.module.exports;
      context[alias] = (globalName && global[alias]) || exports;
    }

    if (pending[id].length < 1) {
      exec(monad.fn, monad);
    }
  });
};

...
/*
 * temporary entry point ~ will be collapsed with def.id()
 *
 * Problem with anonymous module is that it registers relative to THIS module
 */
...
function namespace(filename) {
  ...
  function monad(param) {
    ...
    var value = type == 'function' && (function () {
    
      /*
       * IN PROGRESS ~ STRATEGY TO BE IMPROVED
       * inner exports should not clobber outer exports.
       */
      var exports, result, nested = stack[stack.length - 1] === monad.id;
      
      if (nested) {
        exports = monad.module.exports;
        monad.module.exports = {};
      }
      
      stack.push(monad.id);

      result = exec(param, monad);
      
      if (nested) {
        monad.module.exports = exports;
      }
...
function string(id, monad) {
  ...
  var filename = Module._resolveFilename(id, monad.module);
  
  // cycles are forbidden, no matter what
  var cycle = graph(monad.module.id, filename).resolve(filename);
  assert(!cycle, cycle);  
   ...
   * when a required global is loaded, we can refer to it directly ~ in node, 
   * that's synchronous.
   *
   * TODO ! in browsers, we have the async callback step !
   * 
   * otherwise the exports retrieval should have worked, so assign that to 
   * the alias.
   */

  if (globalName) {
    monad[alias] = global[alias];
  } else {
    monad[alias] = exports;
  }

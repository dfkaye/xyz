...
function Namespace(filename, monad) {
  this.id = filename;
  this.module = cache(filename); // load real module from cache
...
define.id = function (id) {
  ...
  /*
   * bind monad as inner 'define' to the current namespace ~ no .id() call, no 
   * aliasing ~ to support lazier loading; then, return monad.
   */
  monad.namespace = ns;

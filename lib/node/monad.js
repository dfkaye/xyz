 ...
 * function. holds on to a real module.  though laborious this solution repeats 
 * work done by environment to set up require.cache, require.resolve, and bind 
 * require to the encapsulated module correctly.
 */
...
function cache(filename) {
  ...
  if (!cache[filename]) {
    // compiles module into node.js cache but returns module.exports not module
    Module._load(filename);
  }
  
  // get the module out of the cache

...
define.sandbox = function sandbox(fn, context) {
  ...
  return (function () {
    ...
    // re-map the global define to run against the current context monad
    function define(param) {
      context.define(param);
    };
    // however we have to expose these methods on the real define function in 
    // the sandbox
    define.string = g.define.string;
    define.exec = g.define.exec;
    define.sandbox = g.define.sandbox;
    // now expose the fake define as global
    ...
    // restore original global.define in this context
